...
const SearchBar = () => {
  ...
  const renderDocumentation = (post) => {
    const document = post.fields.Document[0];
    if (!document) return null; // Prevent runtime error if no document exists
    ...
    <div>
      ...
      <div className="mt-2 flex rounded-md shadow-sm">
        ...
        <button
          type="button"
          onClick={handleSearch}
          className={`rounded-none rounded-r-md relative -ml-px inline-flex items-center gap-x-1.5 px-3 py-2 text-sm font-semibold ${/^\d{5}$/.test(lotNo) ? 'text-ccDarkBlue ring-gray-300 bg-gray-50' : 'text-gray-400 bg-gray-200 cursor-not-allowed'}`}
          disabled={!/^\d{5}$/.test(lotNo)} // Disable button if input is invalid
        >
          <MagnifyingGlassIcon aria-hidden="true" className="-ml-0.5 h-5 w-5 text-gray-400" />
          Search
        </button>
